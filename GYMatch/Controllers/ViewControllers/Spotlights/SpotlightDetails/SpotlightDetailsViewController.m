...
@synthesize showBookButton, selectedType;
...
- (void)viewDidLoad
{
    ...
    [self initCellViews];
//    
//    [(UITableView *)tableView_Main setContentInset:UIEdgeInsetsMake(-20, 0, -50, 0)];
//    [(UITableView *)tableView_Main setScrollIndicatorInsets:UIEdgeInsetsMake(0, 0, -50, 0)];
    ...
    [(UITableView *)tableView_Main registerNib:nib forCellReuseIdentifier:dayCellNibName];
    
    nib = [UINib nibWithNibName:sessionCellNibName bundle:nil];
    [(UITableView *)tableView_Main registerNib:nib forCellReuseIdentifier:sessionCellNibName];
    ...
    [self showBookingButtonView];
    //[self loadData];
}

- (void)showBookingButtonView {
    
    if ([selectedType isEqualToString:SPOTLIGHT_TYPE_TRAINER]) {
        [button_BookNow setTitle:@"BOOK NOW" forState:UIControlStateNormal];
    } else if ([selectedType isEqualToString:SPOTLIGHT_TYPE_GYMSTAR]) {
        [button_BookNow setTitle:@"JOIN GYM" forState:UIControlStateNormal];
    }
    
    view_BookingButtonView.hidden = showBookButton == YES ? YES : NO;
    tableViewFooterLayoutConstraint.constant = showBookButton == YES ? -64 : 0;
...
-(void)imageClicked : (UITapGestureRecognizer *)gesture {
    ...
    [tableView_Main addSubview:_pictureView.view];
...
- (void)decorate{
    ...
    UITableView *tableView = (UITableView *)tableView_Main;
...
- (void)fillDetailsWithSpotlight:(Spotlight *)aSpotlight{
    ...
    if (![aSpotlight.countmyrate isEqualToString:@"0"] && !(aSpotlight.countmyrate == nil)) {
        [rateBtn setHidden:YES];
    }
    ...
    for (int i = 0; i < 5; i++) {
        if (aSpotlight.rate > i) {
            ...
            [starImageViews[i] setImage:[UIImage imageNamed:@"gray_rating"]];
        }
    ...
    if ([array count] > 0) {
        frame.size.height += [Utility showPrefs:array inView:prefsView withColor:DEFAULT_BG_COLOR] + 70/*+ 20*/;
    }else{
...
- (void)loadData
{
    ...
    [sDC spotlightDetails:self.aSpotlight.ID withSuccess:^(Spotlight *aSpotlight) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [(UITableView *)tableView_Main reloadData];
...
- (IBAction)rateButtonPressed:(id)sender {
    ...
    [rateView setSuccess:^(NSString* rateVal){
        self.aSpotlight.rate = [rateVal floatValue];
        self.aSpotlight.countmyrate = @"1";
...
- (IBAction)bookNow:(id)sender {
    ...
    if ([selectedType isEqualToString:SPOTLIGHT_TYPE_TRAINER]) {
        ...
        calenderViewC.selectedType = SPOTLIGHT_TYPE_TRAINER;
        ...
        NSDictionary *requestDictionary = @{@"user_id": [NSString stringWithFormat:@"%d", (int)[APP_DELEGATE loggedInUser].ID],
                                            @"gym_id" : [NSString stringWithFormat:@"%d", (int)self.aSpotlight.ID],
                                            };
        ...
        [sDC joinGYM:requestDictionary withSuccess:^(NSDictionary *spotlight) {
            ...
            [[[UIAlertView alloc] initWithTitle:@"GYMatch" message:error.userInfo[@"NSLocalizedDescription"] delegate:nil cancelButtonTitle:@"okay" otherButtonTitles:nil] show];
            [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
        }];
        
        
    
//        JoinGYMViewController *joinGYMVC = [[JoinGYMViewController alloc] initWithNibName:@"JoinGYMViewController" bundle:[NSBundle mainBundle]];
//        joinGYMVC.aSpotlight = self.aSpotlight;
//        [self.navigationController pushViewController:joinGYMVC animated:YES];
...
- (IBAction)expandPersonalHour:(UIButton *)sender
{
    ...
    [(UITableView *)tableView_Main reloadData];
    if (sender.selected) {
        if (numOfRows[spotlightType][2] > 1) {
            [(UITableView *)tableView_Main scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:numOfRows[spotlightType][2]-1 inSection:2] atScrollPosition:UITableViewScrollPositionBottom animated:YES];
        }
...
- (IBAction)expandPersonalHalfHour:(UIButton *)sender
{
    ...
    [(UITableView *)tableView_Main reloadData];
    if (sender.selected) {
        if (numOfRows[spotlightType][3] > 1) {
            [(UITableView *)tableView_Main scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:numOfRows[spotlightType][3]-1 inSection:3] atScrollPosition:UITableViewScrollPositionBottom animated:YES];
        }
...
- (IBAction)expandButtonPressed:(UIButton *)sender
{
    ...
    [(UITableView *)tableView_Main reloadData];
    if (sender.selected) {
        [(UITableView *)tableView_Main scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:numOfRows[spotlightType][sender.tag]-1 inSection:sender.tag] atScrollPosition:UITableViewScrollPositionBottom animated:YES];
    }
}
