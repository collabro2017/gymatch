...
- (void)initializator
{
    ...
    self.backgroundColor = [UIColor colorWithRed:0.91f green:0.91f blue:0.91f alpha:1.0f];
...
//        return 1.0f;
    ...
    return MAX(data.insets.top + data.view.frame.size.height + data.insets.bottom, self.showAvatars ? 35 : 0) + 20; // add more to shift bubble upside 
}
...
//        cell.date = data.date;
    ...
    if (self.is_single) {
      //  static NSString* singleCellId = @"SingleChatTable";
        SingleChatTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"SingleChatTable"];
    
        
       NSBubbleData *data = [[self.bubbleSection objectAtIndex:indexPath.section] objectAtIndex:indexPath.row - 1];
        if (cell == nil) {
            cell = [[SingleChatTableViewCell alloc] init];
        }
        cell.delegate = self;
        cell.senderIndex =indexPath.row ;
        //[cell addLineView];
        [cell addLineViewWithHeight:data.view.frame.size.height+60];
        cell.data = data;
        cell.showAvatar = self.showAvatars;
        ...
        if (cell == nil){
            ...
            CGRect cellF = CGRectMake(cell.frame.origin.x, cell.frame.origin.y, tableView.frame.size.width, [self tableView:tableView heightForRowAtIndexPath:indexPath]);
            cell.frame = cellF;
...
-(void) successDeleteChat{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ChatDeleted" object:nil];
}
