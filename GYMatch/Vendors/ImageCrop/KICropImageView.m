...
- (void)setCropSize:(CGSize)size {
    ...
    KICropImageMaskView *mView = [self maskView];
    [mView setCropSize:_cropSize];
...
- (UIImage *)cropImage {
    ...
    CGFloat aWidth =  MAX(_cropSize.width / zoomScale, _cropSize.width) / 2.0;
    CGFloat aHeight = MAX(_cropSize.height / zoomScale, _cropSize.height) / 2.0;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetRGBFillColor(ctx, .4, .4, .4, .6);
