    ...
    if ([self.textFields count] == 0){
        for(UIView *subView in view.subviews){
            if ([subView isKindOfClass:[CustomTextField class]]){
                CustomTextField *textField = (CustomTextField*)subView;
                textField.tag = index;
...
- (void) nextButtonIsClicked:(id)sender{
    ...
    CustomTextField *textField =  [self.textFields objectAtIndex:++tagIndex];
...
- (void)scrollToField
{
    ...
    CGRect textFieldRect = [[_textField superview]  convertRect:_textField.frame toView:scrollView];
    CGPoint point = CGPointMake(0, textFieldRect.origin.y-15);
    [scrollView setContentOffset:point animated:YES];

/*
    CGRect textFieldRect = [_textField  convertRect:_textField.frame toView:[[scrollView superview] window]];
  //  textFieldRect = [[[_textField superview] superview] convertRect:_textField.frame toView:[scrollView superview]];
  ...
  //  CGRect textFieldRect = _textField.frame;
    ...
    float scrollViewVisibleHeight = textFieldRect.origin.y- scrollView.contentOffset.y+64;

    if(scrollViewVisibleHeight > (deviceSize.height - keyBoardHeight))
    {
        float shiftHeight = scrollViewVisibleHeight - (deviceSize.height - keyBoardHeight);
        CGPoint scrollPoint = CGPointMake(0.0, scrollView.contentOffset.y + shiftHeight);
...
- (void) keyboardDidShow:(NSNotification *) notification{
    ...
    if (![_textField isKindOfClass:[CustomTextField class]])
        return;
...
- (void)textFieldDidBeginEditing:(NSNotification *) notification{
    ...
    //  while ([[[self.superview valueForKey:@"superview"] valueForKey:@"superview"] isKindOfClass:[UITableViewCell class]] && self.scrollView == nil) {
